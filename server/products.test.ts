...
import { emptyProductDraft, validateProductDraft } from "../client/src/features/catalog/productDomain";
...
describe("productDomain", () => {
  ...
  it("exige imagem, nome, categoria, preço e estoque", () => {
    ...
    expect(result.errors).toMatchObject({ imageUrl: expect.any(String), name: expect.any(String), category: expect.any(String), price: expect.any(String), stock: expect.any(String) });
  });
  ...
  it("impede duplicidade de nome e SKU", () => {
    createProduct({ name: "Colar Luz", imageUrl: "data:image/png;base64,abc", category: "Colares", price: 100, stock: 1, status: "available", accent: "#c8a86b", sku: "LUZ-01" });
    expect(() => createProduct({ name: "colar luz", imageUrl: "data:image/png;base64,abc", category: "Colares", price: 100, stock: 1, status: "available", accent: "#c8a86b", sku: "LUZ-02" })).toThrow("nome");
    expect(() => createProduct({ name: "Outro", imageUrl: "data:image/png;base64,abc", category: "Colares", price: 100, stock: 1, status: "available", accent: "#c8a86b", sku: "luz-01" })).toThrow("código");
  });
